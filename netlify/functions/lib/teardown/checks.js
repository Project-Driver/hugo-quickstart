...
const FL_LICENSE = {
  ...
  plumbing: { customer: 'a homeowner', re: /\bCFC\s?\d{6,7}\b/i, label: 'CFC (state-certified plumbing) license number', services: '"drain cleaning", "water heater replacement" and "repiping"', schema: 'Plumber', travels: true },
  roofing: { customer: 'a homeowner', re: /\bCCC\s?\d{6,7}\b/i, label: 'CCC (state-certified roofing) license number', services: '"roof repair", "roof replacement" and "storm damage"', schema: 'RoofingContractor', travels: true },
  electrical: { customer: 'a homeowner', re: /\bE[CR]\s?\d{7}\b/i, label: 'EC (state-certified electrical) license number', services: '"panel upgrade", "EV charger install" and "rewiring"', schema: 'Electrician', travels: true },
  'general-contractor': { customer: 'a homeowner', re: /\bCGC\s?\d{6,7}\b/i, label: 'CGC (general contractor) license number', services: '"kitchen remodel", "bathroom remodel" and "additions"', schema: 'GeneralContractor', travels: true },
  pool: { customer: 'a homeowner', re: /\bCPC\s?\d{6,7}\b/i, label: 'CPC (pool contractor) license number', services: '"weekly service", "green pool recovery" and "equipment repair"', schema: 'LocalBusiness', travels: true },
};

const TRADE_WORDS = {
  hvac: { customer: 'a homeowner', problem: 'a broken air conditioner', searches: '"AC not cooling", "no cold air"', example: 'See AC repair pricing', services: '"AC repair", "duct cleaning" and "new install"', schema: 'HVACBusiness', travels: true },
  plumbing: { customer: 'a homeowner', problem: 'a burst pipe', searches: '"water heater leaking", "clogged main line"', example: 'See drain cleaning pricing', services: '"drain cleaning", "water heater replacement" and "repiping"', schema: 'Plumber', travels: true },
  roofing: { customer: 'a homeowner', problem: 'a leaking roof', searches: '"roof leak repair", "storm damage"', example: 'See roof repair pricing', services: '"roof repair", "roof replacement" and "storm damage"', schema: 'RoofingContractor', travels: true },
  electrical: { customer: 'a homeowner', problem: 'no power in half the house', searches: '"power out", "breaker keeps tripping"', example: 'See panel upgrade pricing', services: '"panel upgrade", "EV charger install" and "rewiring"', schema: 'Electrician', travels: true },
  'garage-doors': { customer: 'a homeowner', problem: 'a garage door that will not open', searches: '"garage door stuck", "broken spring"', example: 'See spring replacement pricing', services: '"spring replacement", "opener install" and "new doors"', schema: 'LocalBusiness', travels: true },
  pool: { customer: 'a homeowner', problem: 'a green pool', searches: '"pool pump not working", "green pool cleanup"', example: 'See weekly service pricing', services: '"weekly service", "green pool recovery" and "equipment repair"', schema: 'LocalBusiness', travels: true },
  'pest-control': { customer: 'a homeowner', problem: 'an infestation', searches: '"termites in the wall", "same day pest control"', example: 'See termite treatment pricing', services: '"termite treatment", "rodent removal" and "quarterly service"', schema: 'PestControlService', travels: true },
  landscaping: { customer: 'a homeowner', problem: 'a yard nobody has touched in a month', searches: '"storm cleanup", "tree down"', example: 'See monthly maintenance pricing', services: '"lawn maintenance", "tree trimming" and "landscape design"', schema: 'LocalBusiness', travels: true },
  cleaning: { customer: 'a homeowner', problem: 'a house that has to be ready today', searches: '"same day cleaning", "move out cleaning"', example: 'See move-out cleaning pricing', services: '"move-out cleaning", "deep cleaning" and "recurring service"', schema: 'CleaningService', travels: true },
  'general-contractor': { customer: 'a homeowner', problem: 'a job that has stalled', searches: '"emergency repair", "licensed contractor near me"', example: 'See remodel pricing', services: '"kitchen remodel", "bathroom remodel" and "additions"', schema: 'GeneralContractor', travels: true },
  barbershop: { customer: 'someone', problem: 'a wedding on Saturday and no appointment', searches: '"barber near me open now", "walk in haircut"', example: 'See our cuts and pricing', services: '"fades", "beard trims" and "hot towel shaves"', schema: 'HairSalon', travels: false },
  salon: { customer: 'someone', problem: 'an event this weekend and no appointment', searches: '"salon near me open now", "same day appointment"', example: 'See our services and pricing', services: '"colour", "cuts" and "treatments"', schema: 'HairSalon', travels: false },
  other: { customer: 'someone', problem: 'a problem they want solved today', searches: '"near me open now", "same day"', example: 'See our pricing', services: 'each of your main services', schema: 'LocalBusiness', travels: false },
};
...
function runChecks({ crawl, psi = null, local = null, biz = {} }) {
  ...
  if (!servicePages.length) add({ id: 'no-service-pages', category: 'search', severity: 'high', title: 'No individual service pages', cost: `Google ranks pages, not businesses. Without a page for each service, ${words.services} all compete for one homepage.`, fix: 'One page per service you actually sell, each with its own title, heading, photos and reviews.', effort: 'project', evidence: okPages.some((p) => /^\/(our-)?services?\/?$/i.test(new URL(p.finalUrl).pathname))
      ? `${okPages.length} pages crawled. A services page lists them, but no page covers one service on its own`
      : `${okPages.length} pages crawled, none about a specific service`, product: 'Website Build' });
  const cityPages = okPages.filter((p) => p !== home && /area|locations?|cit(y|ies)|serving|near/i.test(p.finalUrl + ' ' + p.title));
  if (!cityPages.length && city && words.travels) add({ id: 'no-city-pages', category: 'local', severity: 'medium', title: 'No service-area or city pages', cost: `You can rank in ${city}, but the next town over searches "${(biz.trade || 'service').replace(/-/g, ' ')} [their town]" and finds someone else.`, fix: 'A page per city you serve, with real detail: neighborhoods, drive times, jobs done there.', effort: 'project', evidence: 'No page about service areas found', product: 'Local SEO, monthly' });
  ...
  if (!hasLocal) add({ id: 'no-localbusiness-schema', category: 'local', severity: 'high', title: 'No LocalBusiness structured data', cost: 'Structured data is how Google and AI assistants confirm your name, address, phone, hours and service area. Without it they guess.', fix: `Add ${words.schema} JSON-LD with name, address, phone, hours, areaServed and sameAs links to your Google profile and socials.`, effort: 'quick', evidence: types.size ? `Found: ${[...types].filter((t) => t !== '__invalid__').join(', ') || 'none'}` : 'No JSON-LD on any crawled page', product: 'Local SEO, monthly' });
  ...
  if (!telLinks.length) add({ id: 'no-click-to-call', category: 'conversion', severity: 'high', title: 'Phone number is not tappable', cost: 'On a phone, a number that is just text means copy, switch apps, paste. Many do not.', fix: 'Wrap every phone number in a tel: link.', effort: 'quick', evidence: 'No tel: links on homepage', pages: [home.finalUrl], product: 'Get Booked Online' });
    ...
    add({ id: 'phone-buried', category: 'conversion', severity: 'medium', title: 'Phone number is not at the top of the page', cost: `The first thing ${words.customer} with ${words.problem} looks for is the number.`, fix: 'Put the number, tappable, in the header on every page.', effort: 'quick', evidence: `The first tappable number sits ${home.telAtFraction == null ? 'outside the page source' : `${Math.round(home.telAtFraction * 100)}% of the way down the page source`}, and the first number in the text appears ${home.text.search(PHONE_RE)} characters in`, pages: [home.finalUrl], product: 'Get Booked Online' });
  }
  ...
  if (!smsLinks.length) add({ id: 'no-text-us', category: 'conversion', severity: 'low', title: 'No "text us" option', cost: 'Younger customers text first. A sms: link costs nothing.', fix: 'Add a "Text us" link next to the phone number, routed to an inbox someone watches.', effort: 'quick', evidence: 'No sms: links found', product: 'Never Miss a Job' });
  ...
  if (!TRUST_RE.test(allText)) add({ id: 'no-trust-signals', category: 'conversion', severity: 'low', title: 'No licensed / insured / years-in-business line', cost: 'These are the three words every customer checks for before picking between you and the next listing.', fix: 'One line under the headline: "Licensed & insured · Serving [city] since [year]".', effort: 'quick', evidence: 'No trust wording found', product: 'Content Onboarding' });
...
module.exports = { runChecks, PHONE_RE, STREET_RE, FL_LICENSE, LOCAL_TYPES, TRADE_WORDS, plural };
